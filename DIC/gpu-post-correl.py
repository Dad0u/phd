...
def calc_flow_gpu(original_image,
  ...
  correl = crappy.tool.Correl(img0.shape,
      fields=fields)
  correl.setOrig(img0)
  ...
  if hdf_out:
    ...
    fields = np.stack(
        [f.get() for f in correl.getFields(height,width)],axis=-1)
...
if __name__ == '__main__':
  ...
  img_list = sorted(glob("HDD3To/19.01-uni45/epr2/img/*.tif"))[::500]
  img0 = cv2.imread(img_list[0],0)
  calc_flow_gpu(img0,img_list[1:],res_out='res_gpu.hdf')
