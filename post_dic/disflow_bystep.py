...
def calc_flow(file_list,
  ...
  arr = hrel.create_earray(hrel.root, 'table', tables.Float32Atom(),
      (0,height,width,2), expectedrows=len(file_list), filters=filt)
  ...
  if out_res:
    ...
    arr_r = hres.create_earray(hres.root, 'table', tables.Int16Atom(),
        (0,height,width), expectedrows=len(file_list), filters=filt_r)
    names_r = hres.create_earray(hres.root,'names',
        tables.StringAtom(max_size), (0,2),expectedrows=len(file_list)-1)
  ...
  if out_total:
    ...
    arr_t = htot.create_earray(htot.root, 'table', tables.Int16Atom(),
        (0,height,width,2), expectedrows=len(file_list), filters=filt_t)
